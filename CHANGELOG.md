...
- **Added `.gitignore`** — the README's layout tree already listed one, but the file was absent from the package. Without it, `git add .` commits whatever the operating system leaves lying around, and because Netlify publishes everything inside `public/`, a stray `.DS_Store` becomes a publicly fetchable URL on druzzal.com. Covers macOS/Windows/Linux debris, editor state, `.netlify/` CLI state (which stores the site ID linking a local folder to the Netlify site), `__pycache__`, and `.env`/`*.key`/`*.pem` so a credential file can never be committed by accident. Also ignores `*.zip`, since deploy archives are build output, not source.
- **Added `tools/verify-csp.py`** — a deploy gate for the hash-based CSP. Inline scripts (the no-flash theme resolver and each page's JSON-LD) are allowlisted by SHA-256 rather than `'unsafe-inline'`, so editing one character inside any of them changes its hash and the browser silently refuses to run it: the deploy succeeds, the page looks fine to whoever made the edit, and what actually breaks is a flash of the wrong colour scheme or structured data disappearing from Google's index. v18.1-refined hit exactly this scenario. The script reports both directions — a script with no matching hash, and a hash left behind matching no script — and prints the correct value to paste into `_headers`. Verified against a deliberately tampered byte in `about.html`'s JSON-LD: caught, exit code 1.
- **`README.md`** — layout tree now lists `tools/` (previously undocumented) and the `.gitignore` entry is now true. New "Before every deploy" section explains the CSP hash trap and the two commands that catch it.

Audited at the same time, no action needed: CSP hash allowlist matches all 21 inline script blocks across 11 pages with no dead entries; zero broken links, dead fragments or duplicate ids; every `<img>` has `alt`; exactly one `<h1>` per page; zero inline `style=` attributes; no Google Fonts references; canonical/`og:url`/JSON-LD/sitemap all agree on the pretty URLs; every `dateModified` matches its sitemap `lastmod`; `noindex` pages correctly absent from the sitemap; all JSON-LD parses; HTML tag structure balanced on all 11 pages; manifest and sitemap valid; Netlify form markup intact; `.foot-sub` reads "Physician · Public Health Professional" on all 11 pages; `tools/verify-fonts.py` passes.
